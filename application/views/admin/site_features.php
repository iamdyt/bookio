          ...
          <div class="col-md-9 offset-md-1">
            ...
            <?php if (isset($page_title) && $page_title != "Edit"): ?>
              <div class="card list_area">
                <div class="card-header with-border">
                  <?php if (isset($page_title) && $page_title == "Edit"): ?>
                    <span class="card-title pt-2">Edit Service <a href="<?php echo base_url('admin/site_features') ?>" class="pull-right btn btn-sm btn-primary btn-sm"><i class="fa fa-angle-left"></i> <?php echo trans('back') ?></a></span>
                  <?php else: ?>
                    <span class="card-title pt-2"><?php echo trans('features') ?> </span>
                  <?php endif; ?>
                        ...
                        <tbody>
                          <?php $i=1; foreach ($services as $service): ?>
                            <tr id="row_<?php echo html_escape($service->id); ?>">
                                ...
                                <td><?= $i; ?></td>
                                <td><img  class="img-fluid rounded-circle" width="50%" src="<?php echo base_url($service->thumb) ?>"></td>
                                <td><?php echo html_escape($service->name); ?>
                                  <p class="small"><?php echo character_limiter($service->details, 60); ?></p>
                                </td>

                                <td><span class="badge bg-secondary"><?php echo html_escape($service->orders); ?></span></td>
                                
                                <td class="actions">
                                    <div class="btn-group">
                                      <button type="button" class="btn mt-3" data-toggle="dropdown" aria-expanded="false">
